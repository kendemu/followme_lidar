...
const double follow_speed = 0.6;        // 追跡するときの基準速度
...
const double gain_proportion  = 0.1;  // 比例航法のゲイン
const double linear_max_speed   = 0.6;
const double turn_max_speed     = 0.6; // 3.14;
               ...
               double laser_angle_max, double laser_angle_increment)
{
  ...
  lidar_gray_image = cv::Scalar::all(0); // 画面を白くする
...
int findLegs(cv::Mat input_image, Object *object, cv::Mat display_image,
	     ...
	     double m10_min, double m10_max, double diff_x,  double diff_y)
{
  ...
  for(unsigned int cn=0; cn<contours.size(); cn++)
	...
	for (int j=rect.tl().x; j < rect.tl().x + rect.width; j++) {
      ...
      double intensity_min = 120, intensity_max = 125;
      //if (!((intensity > intensity_min) && (intensity < intensity_max))) continue;  
...
int main(int argc, char* argv[])
{
  ...
  cv::Mat img2(cv::Size(IMAGE_WIDTH, IMAGE_HEIGHT), CV_8U, cv::Scalar(255));
  ...
  cv::VideoWriter writer2("videofile2.avi", CV_FOURCC_MACRO('X', 'V', 'I', 'D'), 30.0, \
			 cv::Size(IMAGE_WIDTH, IMAGE_HEIGHT), false);
  ...
  while(ros::ok()) {
    ...
    cv::Mat d1_img, d3_img, e1_img, e2_img, e3_img,e4_img, tmp_img,final_img;
   ...
   lidar_bin_image = ~lidar_bin_image;
    ...
    //obj2_num = findLegs(e2_img, obj2, white_image2, "Circle 2", blue,
    //		8,32,9, 30, 0.2, 2, 110, 350, 3000, 10000, 2, 2);

    //cv::namedWindow( "Erode2", CV_WINDOW_AUTOSIZE );
    //cv::imshow("Erode2",white_image2);

    //cv::namedWindow( "e3_img", CV_WINDOW_AUTOSIZE );
    //cv::imshow("e3 img",e3_img);
    ...
    cv::namedWindow( "Lidar gray image", CV_WINDOW_AUTOSIZE );
    cv::Mat dst_img1 = ~lidar_gray_image;
    cv::imshow("Lidar gray image",dst_img1);
    ...
    //calcHumanPos(obj2_num,obj2, &tmp_distance, &tmp_angle);
    ...
    double angle_change;
    // 5度以内のときは回転しない                                                          
    if (fabs(human.angle) > 5.0 * M_PI/ 180.0) {
