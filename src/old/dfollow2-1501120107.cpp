...
// 150112: 高速化のためlidarのデータをgray scaleに格納。輝度はintensity情報
               ...
               double laser_angle_max, double laser_angle_increment)
{
  ...
  lidar_gray_image = cv::Scalar::all(255);
...
void calcHumanPos(int object_num,  Object *object, double *dist, double *ang)
{
  ...
  case 1:
    {
      // 1番近い物体を探す                                                                              
      min1_dist = (object[0].pos.x - IMAGE_WIDTH/2) * (object[0].pos.x -IMAGE_WIDTH/2)
  ...
  default: // 2個以上                                                                                   
	...
	if (image2_dist < min2_dist) {
      ...
      // ２個の距離が0.5m以上離れていたら誤検出と考えて一番近い物体を人に位置と考える     
	 ...
	 *ang  = ((atan2(min1_point.x - IMAGE_WIDTH/2, IMAGE_HEIGHT/2 - min1_point.y))
	 ...
	 cv::circle(lidar_image,min1_point,5,blue,2);
	 cv::circle(lidar_image,min2_point,5,blue,2);
      }
      else {
	*dist = sqrt(min1_dist) / mToPixel;
	*ang  = atan2(min1_point.x - IMAGE_WIDTH/2, IMAGE_HEIGHT/2 - min1_point.y);
...
int main(int argc, char* argv[])
{
  ...
  while(ros::ok()) {
	...
	for (int i=0; i < obj_num; i++) {
	  image1_dist = (obj[i].pos.x - IMAGE_WIDTH/2) * (obj[i].pos.x -IMAGE_WIDTH/2)
	    + (obj[i].pos.y - IMAGE_HEIGHT/2) * (obj[i].pos.y -IMAGE_HEIGHT/2);
	  if (image1_dist < min1_dist) {
	    min1_num  = i;
	    min1_dist = image1_dist;
	    min1_point.x = obj[i].pos.x;
	    min1_point.y = obj[i].pos.y;
	  }
	}
	human.distance = sqrt(min1_dist) / mToPixel;
        human.angle    = atan2(min1_point.x - IMAGE_WIDTH/2, IMAGE_HEIGHT/2 - min1_point.y);
	break;
      }
    default: // 2個以上
      {
	...
	for (int i=0; i < obj_num; i++) {
	  if (i == min1_num) continue;
	  image2_dist = (obj[i].pos.x - IMAGE_WIDTH/2) * (obj[i].pos.x -IMAGE_WIDTH/2)
	    + (obj[i].pos.y - IMAGE_HEIGHT/2) * (obj[i].pos.y -IMAGE_HEIGHT/2);
	  if (image2_dist < min1_dist) {
	    min2_num  = i;
	    min2_dist = image2_dist;
	    min2_point.x = obj[i].pos.x;
	    min2_point.y = obj[i].pos.y;
	  }
	}

	// ２個の距離が0.5m以上離れていたら誤検出と考えて一番近い物体を人に位置と考える
	double dist = (min1_point.x - min2_point.x) *  (min1_point.x - min2_point.x) 
	  + (min1_point.y - min2_point.y) *  (min1_point.y - min2_point.y);
	if (dist < 0.5) {
	  human.distance 
	    = (sqrt(min1_dist) + sqrt(min2_dist))/(mToPixel * 2);
	  human.angle 
	    = ((atan2(min1_point.x - IMAGE_WIDTH/2, IMAGE_HEIGHT/2 - min1_point.y))
	       +(atan2(min2_point.x - IMAGE_WIDTH/2, IMAGE_HEIGHT/2 - min2_point.y)))/2;
	}
	else {
	  human.distance = sqrt(min1_dist) / mToPixel;
