...
const double gain_turn  = 0.1;
const double linear_max_speed   = 0.5;
const double turn_max_speed     = 0.5;
	      ...
	      const string& winname, cv::Scalar color)
{
  ...
  for(unsigned int cn=0; cn<contours.size(); cn++)
	...
	if (!((ratio >= ratio_min) && (ratio <= ratio_max))) continue;
      ...
      // 面積による除外(m00)
      int area_min = 50, area_max = 200; // pixel
      ...
      // m10
      int m10_min = 10000, m10_max = 50000;
...
void calcHumanPos(int object_num,  Object *object, double *dist, double *ang)
{
  ...
  default: // 2個以上                                                                                   
	...
	// 左右４５度以内を脚と考える。それより外は追わない.
	if ((object[i].pos.y - IMAGE_HEIGHT/2 < 0)
	    && (object[i].pos.y - IMAGE_HEIGHT/2) / diff1 >= -1.0) continue; 
	...
	double diff2 = fabs(object[i].pos.x - IMAGE_WIDTH/2);
        ...
        // 左右４５度以内を脚と考える。それより外は追わない. 
        if ((object[i].pos.y - IMAGE_HEIGHT/2 < 0)
            && (object[i].pos.y - IMAGE_HEIGHT/2) / diff2 >= -1.0) continue;
