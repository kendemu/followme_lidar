	      ...
	      const string& winname, cv::Scalar color)
{
  ...
  for(unsigned int cn=0; cn<contours.size(); cn++)
      ...
      for(int j=circleROI.y; j<circleROI.y+circleROI.height; ++j)
	{
      ...
      Point2f point;
      point.x = mom.m10/mom.m00;                                                                                                   point.y = mom.m01/mom.m00;   
      
      const float ratioThreshold = 0.25f; //0.4f, 0.1625f;                                                                 
      float ratio;                                                                                                                 if (sumCirclePixel == 0) ratio = 0.0f;                                                                                       else ratio =  sumCirclePixelMarked/sumCirclePixel;                                                                      
      ...
      // 重心位置と面積のパラメータを調整しなければならない
      if((point.y > center.y + 0.2) && (fabs(point.x - center.x) < 0.2 * radius)
	     && (mom.m00 > 50) && (mom.m00 < 150))  {
...
int main(int argc, char* argv[])
{
  ...
  while(ros::ok()) {
    ...
    // 調整するパラメータ１番目と２番目。脚断面の半径のピクセル数
    obj1_num = findLegs(4, 15, e1_img, obj1, lidar_image, "Cirlce 1", red);





    //obj1_num = findLegs(leg_radius, 3 * leg_radius, e1_img, obj1, lidar_image, "Cirlce 1", red);
