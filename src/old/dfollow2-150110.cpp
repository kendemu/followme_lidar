...
// 平均画像。ちらつきを低減するため
cv::Mat lidar_ave_image(cv::Size(IMAGE_WIDTH, IMAGE_HEIGHT),
               ...
               double laser_angle_max, double laser_angle_increment)
{
  ...
  /* for (int j=0; j < lidar_image.rows; j++) {
    for (int i = 0; i < lidar_image.cols; i++) {
      lidar_image.data[j*lidar_image.step+i*lidar_image.elemSize()  ] = 0; //255;
      lidar_image.data[j*lidar_image.step+i*lidar_image.elemSize()+1] = 0; // 255;
      lidar_image.data[j*lidar_image.step+i*lidar_image.elemSize()+2] = 0; // 255;
    }
    } */

  for (int j=0; j < lidar_ave_image.rows; j++) {                                                       for (int i = 0; i < lidar_ave_image.cols; i++) {                                                  
      if (lidar_ave_image.data[j*lidar_ave_image.step+i*lidar_ave_image.elemSize() ]  >128)
	lidar_ave_image.data[j*lidar_ave_image.step+i*lidar_ave_image.elemSize() ] =255;
      else 
      lidar_ave_image.data[j*lidar_ave_image.step+i*lidar_ave_image.elemSize() ] += 50;

      /* if (lidar_ave_image.at<int>(j,i)  > 255)
	lidar_ave_image.at<int>(j,i) = 255;
      else
      lidar_ave_image.at<int>(j,i)++; */
    }                                                                                             
  } 
  ...
  for (int j = center - search_lines/2; j <= center + search_lines/2; j++) {
	...
	= value; 
      ...
      // 平均画像
      lidar_ave_image.data[y*lidar_ave_image.step+x*lidar_ave_image.elemSize()  ] -= 10;
      if (lidar_ave_image.data[y*lidar_ave_image.step+x*lidar_ave_image.elemSize() ] < 0)
	lidar_ave_image.data[y*lidar_ave_image.step+x*lidar_ave_image.elemSize()  ]  
	  = 0;

      //printf("ave=%f\n", tmp);
      //if (lidar_ave_image.data[y*lidar_image.step+x*lidar_image.elemSize()+2] > 128)
      // 	lidar_ave_image.data[y*lidar_image.step+x*lidar_image.elemSize()+2] = 255;
...
int main(int argc, char* argv[])
{
  ...
  // OpenCV
  // 幅320px、高さ500px、3チャンネルのインスタンスを生成する
  ...
  while(ros::ok()) {
    ...
    cv::imshow("lidar ave image", lidar_ave_image);
    ...
    // グレースケールに変換する
    //cvtColor(lidar_ave_image, lidar_gray_image, CV_RGB2GRAY);
    //cvtColor(lidar_image, lidar_gray_image, CV_RGB2GRAY);


    // 平滑化
    //cv::Mat lidar_blur_image;
    //cv::GaussianBlur(lidar_gray_image, lidar_blur_image, cv::Size(5,5), 5, 5);


    // gray scale -> binary                                                           
    //cv::threshold(lidar_gray_image, lidar_bin_image, 0, 255,
    //              cv::THRESH_BINARY| cv::THRESH_OTSU);
    cv::threshold(lidar_ave_image, lidar_bin_image, 0, 255,
                  cv::THRESH_BINARY| cv::THRESH_OTSU);
    ...
    //cv::imshow("e1 image", e1_img);
    //cv::imshow("e2 image", e2_img);
    ...
    cv::HoughCircles(lidar_gray_image, circles, CV_HOUGH_GRADIENT,
		     1, 0.3 * mToPixel, 100, 10, (int) (0.04 * mToPixel), (int) (0.15 * mToPixel) );
    for( size_t i = 0; i < circles.size(); i++ )
      {
	cv::Point center(cvRound(circles[i][0]), cvRound(circles[i][1]));
	int radius = cvRound(circles[i][2]);
	// 円の中心を描画します．
	cv::circle( lidar_gray_image, center, 3, Scalar(0,255,0), -1, 8, 0 );
	// 円を描画します．
	cv::circle( lidar_gray_image, center, radius, Scalar(0,0,255), 3, 8, 0 );
      }
    cout << "Detected circles=" << circles.size() << endl;
    cv::imshow( "circles", lidar_gray_image); */
